...
RACE_INFO_LITERAL = Literal[RaceInfo.list, RaceInfo.beforeinfo, RaceInfo.result]
# dataディレクトリのpath
...
def extract_data_from_localHTML_result(
    race_num: int = 1,
    joh_code: int = 4,
    yearday_8keta: datetime = datetime.datetime(2024, 1, 3),
):
    """ローカルのhtmlファイルの直前情報からデータを取得します"""
    _, file_path = create_data_path_str(
        race_num, joh_code, yearday_8keta, RaceInfo.result.value
    )
    df_race = pd.DataFrame()
    if not os.path.exists(file_path):
        return df_race
    with codecs.open(file_path, "r", "utf-8") as file:
        ...
        # 順位情報
        racers_rank = {}
        rank_html = soup.find_all("div", class_="grid_unit")
        if len(rank_html) == 0:
            return pd.DataFrame()
        each_racer = rank_html[0].find_all("tbody")
        for one_racer_result in each_racer:
            one_racer_result_list = one_racer_result.find_all("td")
            try:
                rank = int(zenkaku_to_hankaku(one_racer_result_list[0].get_text()))
            except Exception:
                rank = None
            souban = int(one_racer_result_list[1].get_text())
            racers_rank[souban] = rank
        # 各レーサーのデータをそれぞれ1レコードとしてDataframeを作成する
        df_race = pd.DataFrame()
        for one_racer in racers_rank:
            data = np.asarray(
                race_info + [one_racer] + [racers_rank[one_racer]]
            ).reshape(1, -1)
            df_temp = pd.DataFrame(data, columns=CATEGORY_SET_RACERESULT_MP)
            df_race = pd.concat([df_race, df_temp])
        return df_race
...
def get_merged_dataframe(
    ...
    try:
        ...
        if len(beforeinfo) == 0:
            return beforeinfo
        raceresult = extract_data_from_localHTML_result(
            race_num=race_num, joh_code=joh_code, yearday_8keta=yearday_8keta
        )
        if len(raceresult) == 0:
            return raceresult
    ...
    # racelist と beforeinfoの情報をmergeしたものと、 raceresult をmerge
    category_common = []
    for result_col in raceresult.columns:
        if result_col in merged_df.columns:
            category_common.append(result_col)
            merged_df[result_col] = merged_df[result_col].astype(str)
            raceresult[result_col] = raceresult[result_col].astype(str)
    merged_df = pd.merge(merged_df, raceresult, on=category_common, how="inner")
    return merged_df


def wait_random(min_second, max_second):
...
if __name__ == "__main__":
    ...
    end_date = datetime.datetime(2024, 6, 15)
    # レース場コード
    for jcd_code in range(1, 25):
        # 日付
        ...
        while target_date <= end_date:
            ...
            for race_num in range(1, 13):
                for info_kind in RaceInfo:
                    # 出走表/結果表/事前情報をダウンロード
                    is_available = query_html_and_save(
                        race_num=race_num,
                        joh_code=jcd_code,
                        yearday_8keta=target_date,
                        info_kind=info_kind.value,
                    )
                    # レース情報が存在しなければ以後の処理を行わない
                    if not is_available:
                        print("error info_kind")
                        break
                    # wait_random(0, 2)
                if not is_available:
                    print(f"error race_num {race_num}")
                    break
            target_date += datetime.timedelta(days=1)

    # for jcd_code in range(0, 25):
    ...
    #         for race_num in range(1, 13):
    #             # 情報が存在した場合はデータフレームに変換
    #             target_df = get_merged_dataframe(race_num, jcd_code, target_date)
    #             if len(target_df) == 0:
    #                 continue
    #             df_total = pd.concat([df_total, target_df])
    #         target_date += datetime.timedelta(days=1)
    #         # データが存在しない場合はスキップ
    #         if len(df_total) == 0:
    #             continue
    #         # Dataframeデータを逐次CSVに書き込み
    #         output_csv_path = (
    #             str(jcd_code).zfill(2)
    #             + "_"
    #             + start_date.strftime("%Y%m%d")
    #             + "-"
    #             + end_date.strftime("%Y%m%d")
    #             + output_csv_tail_name
    #         )
    #         if os.path.exists(output_csv_path):
    #             df_total.to_csv(output_csv_path, mode="a", index=False, header=False)
    #         else:
    #             df_total.to_csv(output_csv_path, mode="w", index=False, header=True)
